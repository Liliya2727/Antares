...
async function checkfpsged() {
    let {
        errno: c,
        stdout: s
    } = await executeCommand("cat /data/adb/.config/AZenith/fpsged");
...
async function checkBypassChargeStatus() {
    let {
        errno: c,
        stdout: s
    } = await executeCommand("cat /data/adb/.config/AZenith/bypass_charge");
...
async function checkDThermal() {
    let {
        errno: c,
        stdout: s
    } = await executeCommand("cat /data/adb/.config/AZenith/DThermal");
...
async function checkschedtunes() {
    let {
        errno: c,
        stdout: s
    } = await executeCommand("cat /data/adb/.config/AZenith/schedtunes");
...
async function checkmalisched() {
    let {
        errno: c,
        stdout: s
    } = await executeCommand("cat /data/adb/.config/AZenith/malisched");
